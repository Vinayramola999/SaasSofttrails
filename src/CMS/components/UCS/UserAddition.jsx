...
const UserAddition = () => {
  ...
  const moduleNames = useMemo(() => {
    ...
    return [
      ...new Set(
        allModulesData
          .filter((item) => item.applicationName === formData.name)
    ...
    return [
      ...new Set(
        allModulesData
          .filter(
            (item) =>
              item.applicationName === formData.name &&
              item.moduleName === formData.moduleName
          )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newItem =
          created && typeof created === "object" && Object.keys(created).length
            ? created
            : fallbackItem;
        ...
        setTableData((prev) =>
          Array.isArray(prev)
            ? [newItem, ...prev.filter((i) => i.id !== newItem.id)]
            : [newItem]
        );
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <CustomizableSelect
                ...
                id="name"
                value={formData.name}
                customValue={customFields.name}
                onChange={handleChange}
                onCustomChange={handleCustomFieldChange}
              ...
              <CustomizableSelect
                ...
                customValue={customFields.moduleName}
                onChange={handleChange}
                onCustomChange={handleCustomFieldChange}
                options={moduleNames}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <CustomizableSelect
                ...
                customValue={customFields.subModuleName}
                onChange={handleChange}
                onCustomChange={handleCustomFieldChange}
                options={subModuleNames}
                disabled={!formData.moduleName}
                placeholder="Enter new sub module name"
              />
              <div>
                <label
                  htmlFor="tableName"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label
                  htmlFor="uniqueIdentifierName"
                  className="block text-gray-700 font-medium mb-2"
                >
                  Unique Identifier<span className="text-red-500 ml-1">*</span>
                </label>
                <input
                  type="text"

...
const UpdateAccess = () => {
    ...
    const handleApiAccessChange = async (apiName, isRole = false) => {
        ...
        if (apiName === 'update_access') {
            ...
            if (!hasAmsAccess) {
                try {
                    const response = await axios.get('https://devapi.softtrails.net/saas/test/access', {
                        headers: {
                            'Authorization': `Bearer ${sessionStorage.getItem('token')}`,
                        },
                    });
            ...
            return; // Exit as no further processing is needed for `update_access`
        }
        // Handle API call for other APIs
        ...
        try {
            const response = await axios.put(
                'https://devapi.softtrails.net/saas/test/access/update_access',
                {
                    ...
                    api_access: selectedApiAccess, // Can be an empty array
                },
                {
                    headers: {

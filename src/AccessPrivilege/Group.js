...
const UpdateAccess = () => {
    ...
    const handleEmailChange = async (e) => {
        ...
        if (userId) {
            try {
                const accessResponse = await axios.get(`https://devapi.softtrails.net/saas/test/access/access/${userId}`, {
                    headers: { 'Authorization': `Bearer ${sessionStorage.getItem('token')}` },
                });

                const filteredAccess = accessResponse.data.filter(access => access.user_id === parseInt(userId));
                ...
                showAlert('Error', 'No Access is available for the provided User', 'error');
            }
        } else {
            setApiAccess([]);
            setRoles([]);
    ...
    const handleSubmit = async (e) => {
        ...
        const selectedRolesArray = Object.keys(selectedRoles).filter(role => selectedRoles[role]);
        ...
        try {
            const response = await axios.put('https://devapi.softtrails.net/saas/test/access/update_access', payload, {
                headers: {
            ...
            if (response.status === 200) {
                ...
                showAlert('Error', 'User not found. Please check the email entered.', 'error');
            } else {
            ...
            if (error.response) {
                if (error.response.status === 403) {
                    showAlert('Error', 'You do not have permission to perform this action.', 'error');
    ...
    useEffect(() => {
        ...
        fetch("https://devapi.softtrails.net/saas/test/role", {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        })
    ...
    const handleDownloadExcel = () => {
        ...
        const roleName = roles.find((r) => r.role_id === parseInt(selectedRole1))?.role || 'Role';
    ...
    const handleGroupChange1 = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`https://devapi.softtrails.net/saas/test/access/role-users?role=${roleName}`, {
                headers,
            });
            ...
            // Fetch all users
            const userRes = await fetch(`https://devapi.softtrails.net/saas/test/users`, {
                headers,
            });
        ...
        <div className="p-4 sm:p-6 w-full flex flex-col lg:flex-row gap-6">
            ...
            <div className="bg-white w-full lg:w-1/2 h-[500px] rounded-xl shadow-md p-4 flex flex-col border border-blue-300">
                <div className="flex flex-col sm:flex-row sm:items-center mt-2 mb-4">
                    <label htmlFor="role" className="block text-sm font-medium text-gray-700 mb-2 sm:mb-0 sm:mr-4">
                        Select Group:
                    </label>
                    <select
                        id="role"
                    ...
                    <button onClick={handleDownloadExcel} className="ml-2">
                        <img src={Excel} alt="Excel" className="w-8 h-8 cursor-pointer hover:scale-105 transition" />
                    </button>
                </div>

...
import { FaHome, FaIndustry, FaProjectDiagram, FaCalculator, FaBoxOpen, FaClipboardCheck, FaArrowRight, FaCogs } from 'react-icons/fa';
import ProfileDropdown from './ProfileDropdown';
import ProfilePart from './ProfilePart';
import axios from 'axios';
import { MAIN_API_BASE } from '../config/apiBase';

const AssemblyCard = ({ title, icon, onClick, colorClass }) => (
    <button
        onClick={onClick}
...
const ProductAssembly = () => {
    ...
    useEffect(() => {
        const getUserAccessibleCard = async () => {
            // We'll check for the main PAL access and potentially sub-modules if strictly required.
            // For now, checking the main module key "PAL" and enabling all sub-features if access is granted.
            // You can refine this list if specific API keys exist for sub-pages.
            const cardTitles = ['PAL'];
            if (!token) return;
            ...
            try {
                ...
                if (data['PAL']) {
                    ...
                    available['Approvals'] = 'Approvals';
                    available['Estimation'] = 'Estimation';
    ...
    const getCardConfig = (key) => {
        switch (key) {
            case "Project": 
                return { title: "Projects", icon: <FaIndustry />, color: "from-blue-500 to-cyan-400", path: "Project" };
            case "Allocation": 
                return { title: "Allocation Request", icon: <FaProjectDiagram />, color: "from-purple-500 to-fuchsia-400", path: "AllocationRequest" };
            case "Approvals": 
                return { title: "Process Approvals", icon: <FaClipboardCheck />, color: "from-amber-500 to-orange-400", path: "Processtab" };
            case "Estimation": 
                return { title: "Production Estimation", icon: <FaCalculator />, color: "from-emerald-500 to-teal-400", path: "ProjectEstimation" };
            case "Output": 
                return { title: "Production Output", icon: <FaBoxOpen />, color: "from-rose-500 to-pink-400", path: "ProductionOutput" };
            default: 
                return { title: key, icon: <FaCogs />, color: "from-gray-500 to-gray-400", path: "#" };
        }
    }
    ...
    // Ordered keys to maximize layout consistency
    const orderedKeys = ['Project', 'Allocation', 'Approvals', 'Estimation', 'Output'];
             ...
             <div className="bg-custome-blue rounded-lg w-full px-4 py-3 flex justify-between items-center shadow-lg sticky top-0 z-20 mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-white text-xl sm:text-2xl font-bold tracking-wide">
                        Product <span className="font-light opacity-80 text-lg">Assembly Line</span>
                    </h1>
                </div>
                ...
                <div className="w-full md:w-3/4 lg:w-4/5 flex flex-col">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 h-full overflow-y-auto custom-scrollbar">
                        <div className="mb-8 border-b border-gray-100 pb-4">
                            <h2 className="text-3xl font-extrabold text-gray-800 tracking-tight">Assembly Modules</h2>
                            <p className="text-gray-500 mt-2 text-lg">Manage production, projects, and assembly workflows.</p>
                        </div>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
                            {orderedKeys.map(key => {
                                if (!availableBtn[key]) return null;
                        ...
                        {Object.keys(availableBtn).length === 0 && (
                             <div className="flex flex-col items-center justify-center h-64 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                <p className="text-lg font-medium">No Assembly modules available.</p>

...
import { FaHome, FaClipboardList, FaUserCog, FaCoins, FaTachometerAlt, FaTh, FaArrowRight, FaServer } from 'react-icons/fa';
import ProfileDropdown from './ProfileDropdown';
import ProfilePart from './ProfilePart';
import axios from 'axios';
import { MAIN_API_BASE } from '../config/apiBase';

const AssetCard = ({ title, icon, onClick, colorClass }) => (
    <button
        onClick={onClick}
...
const AssetManagement = () => {
    ...
    useEffect(() => {
        const getUserAccessibleCard = async () => {
            const cardTitles = ['Approvals', 'RepoAllTab', 'Category', 'Valuation'];
            if (!token) return;
            ...
            try {
                ...
                // Always add Dashboard and Asset History if we are here (assuming ASM access)
                available['Dashboard'] = 'Dashboard';
                available['AssetHistory'] = 'AssetHistory';
    ...
    const getCardConfig = (key) => {
        switch (key) {
            case "Dashboard": 
                return { title: "Dashboard", icon: <FaTachometerAlt />, color: "from-indigo-500 to-blue-500", path: "Reports" };
            case "AssetHistory": 
                return { title: "Asset History", icon: <FaClipboardList />, color: "from-purple-500 to-indigo-400", path: "AssetHistory" };
            case "Category": 
                return { title: "Asset Category", icon: <FaClipboardList />, color: "from-pink-500 to-rose-400", path: "CategoryTab" };
            case "Approvals": 
                return { title: "Approvals", icon: <FaUserCog />, color: "from-amber-500 to-orange-400", path: "AllTab" };
            case "Valuation": 
                return { title: "Asset Valuation", icon: <FaCoins />, color: "from-emerald-500 to-teal-400", path: "Depreciation" };
            case "RepoAllTab": 
                return { title: "Asset Inventory", icon: <FaServer />, color: "from-cyan-500 to-sky-400", path: "RepoAllTab" };
            default: 
                return { title: key, icon: <FaTh />, color: "from-gray-500 to-gray-400", path: "#" };
        }
    }
    ...
    const orderedKeys = ['Dashboard', 'AssetHistory', 'Category', 'RepoAllTab', 'Approvals', 'Valuation'];
             ...
             <div className="bg-custome-blue rounded-lg w-full px-4 py-3 flex justify-between items-center shadow-lg sticky top-0 z-20 mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-white text-xl sm:text-2xl font-bold tracking-wide">
                        Asset <span className="font-light opacity-80 text-lg">Management</span>
                    </h1>
                </div>
                        ...
                        >
                            ...
                            />
                             <div className="ml-2 hidden sm:block text-left">
                                ...
                                <p className="text-[10px] text-green-500 leading-none mt-0.5">Online</p>
                            </div>
                        </button>
                        {isDropdownOpen && (
                ...
                <div className="w-full md:w-3/4 lg:w-4/5 flex flex-col">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 h-full overflow-y-auto custom-scrollbar">
                        <div className="mb-8 border-b border-gray-100 pb-4">
                            <h2 className="text-3xl font-extrabold text-gray-800 tracking-tight">Asset Modules</h2>
                            <p className="text-gray-500 mt-2 text-lg">Manage your organization's assets efficiently.</p>
                        </div>
                        ...
                        {Object.keys(availableBtn).length === 0 && (
                             <div className="flex flex-col items-center justify-center h-64 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                <p className="text-lg font-medium">No Asset modules available.</p>

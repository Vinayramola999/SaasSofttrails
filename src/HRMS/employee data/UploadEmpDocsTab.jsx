import axios from 'axios';
import React, { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import RevisionLetter from './RevisionLetter';
import SalarySlip from './SalarySlip';
import HREmpDocument from './HREmpDocument';
import {MAIN_API_BASE} from '../../config/apiBase';

const UploadEmpDocsTab = () => {
  const [userData, setUserData] = useState(null);
  const [loading, setLoading] = useState(null);
  const [activeTab, setActiveTab] = useState("revision");
  const userId = sessionStorage.getItem('userId');
  const [tabs, setTabs] = useState([{ id: "revision", label: "Revision Letter" }]);

  const getToken = () => {
    const token = sessionStorage.getItem('token');
    return token;
  };
  const token = getToken();

  useEffect(() => {
    const userId = sessionStorage.getItem('userId');
    if (userId) {
      const fetchUserData = async () => {
        try {
          const response = await axios.get(`${MAIN_API_BASE}/users/id_user/${userId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            }
          );
          if (response.data?.user) {
            setUserData(response.data.user);
          }
        } catch (error) {
          console.error('Error fetching user data:', error);
        }
      };
      fetchUserData();
    }
  }, [token, userId]);

  useEffect(() => {
    const checkAMSAccess = async () => {
      setLoading(true);
      try {
        const userId = sessionStorage.getItem('userId');
        const token = sessionStorage.getItem('token');
        if (!userId || !token) {
          console.error('userId or token is missing');
          return;
        }
        const response = await axios.get(`${MAIN_API_BASE}/access/access/${userId}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
        const userAccess = response.data;
        const hasRevisionLetterAccess = userAccess.some(access => access.api_name === 'RevisionLetter');
        const hasSalarySlipAccess = userAccess.some(access => access.api_name === 'SalarySlip');
        const hasDocumentAccess = userAccess.some(access => access.api_name === 'Document');
        const accessibleTabs = [];
        if (hasRevisionLetterAccess) accessibleTabs.push({ id: "revision", label: "Revision Letter" });
        if (hasSalarySlipAccess) accessibleTabs.push({ id: "salaryslip", label: "Salary Slip" });
        if (hasDocumentAccess) accessibleTabs.push({ id: "document", label: "Document" });
        setTabs(accessibleTabs);
        setActiveTab(accessibleTabs[0].id);
      } catch (error) {
        console.error('Error during API call: ', error);
      } finally {
        setLoading(false);
      }
    };
    checkAMSAccess();
  }, []);

  return (
    <div className='flex flex-col w-full'>
      <div className="flex flex-col w-[100%]">
        <div className="flex flex-col  gap-4">
          <div className="flex gap-2 w-[50%] rounded-full p-1 relative">
            <motion.div
              layoutId="activeTab"
              className="absolute top-1 bottom-1 left-0 bg-gradient-to-r from-blue-500 to-blue-800 rounded-full transition-all duration-300"
              style={{ width: `calc(100% / ${tabs.length})`, left: `${(tabs.findIndex((t) => t.id === activeTab) * 100) / tabs.length}%`, }}
              transition={{
                type: "spring",
                stiffness: 600,
                damping: 20,
              }}
            />

            {tabs.map((tab) => (
              <button key={tab.id} className={`relative flex-1 px-4 py-2 rounded-full text-center font-medium transition-all duration-300 z-10 ${activeTab === tab.id ? "text-white" : "text-black"}`}
                onClick={() => setActiveTab(tab.id)}
              >
                {tab.label}
              </button>
            ))}
          </div>

          {/* Tab Content */}
          <div className="flex-grow border-gray-300 h-screen">
            {activeTab === "revision" && <RevisionLetter />}
            {activeTab === "salaryslip" && <SalarySlip />}
            {activeTab === "document" && <HREmpDocument />}
          </div>
        </div>
      </div>
    </div>
  );
};
export default UploadEmpDocsTab;
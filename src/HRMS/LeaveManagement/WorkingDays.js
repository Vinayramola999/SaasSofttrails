...
//       try {
//         const res = await fetch("https://devapi.softtrails.net/hrms/test/leave/working-days", {
//           method: "GET",
...
//           // Filter by selected year type (calendar / financial)
//           const filtered = data.filter(
//             (entry) =>
//               entry.year_type.toLowerCase() === yearType.toLowerCase()
//           );

//           // Reset days before applying filtered data
//           const updatedDays = { ...emptyDays };

//           filtered.forEach((entry) => {
//             let selectedWeeks;
//             if (entry.weeks.includes("all")) {
//               selectedWeeks = [...weeks]; // Select all weeks
//             } else {
...
//     fetch("https://devapi.softtrails.net/hrms/test/leave/days", {
//       method: "POST",
//       headers: { "Content-Type": "application/json" },
...
//       <select value={yearType} onChange={(e) => setYearType(e.target.value)} className="w-1/3 border border-gray-300 rounded-md p-2 mb-4" >
//         <option value="" disabled> Select Year Type  </option>
...
//         className="mt-4 w-1/3 bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600"
//       >
//         Submit
//       </button>
...
const WorkingDays = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://devapi.softtrails.net/hrms/test/leave/days", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${sessionStorage.getItem("token")}`, // ✅ Added token

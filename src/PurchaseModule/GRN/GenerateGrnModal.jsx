...
//       const grnStatus = allQcPassed ? "Pass QC" : "Fail QC";
      ...
      try {
        ...
        if (record) {
          ...
          const mappedRows = dataArray.map((it) => ({
            asset_name: it.asset_name || it.name || "",
            quantity: Number(it.quantity ?? it.qty ?? 0),
            received_quantity: Number(it.received_quantity ?? it.quantity ?? it.qty ?? 0),
            accepted_quantity: Number(it.accepted_quantity ?? it.quantity ?? it.qty ?? 0),
            ...
            qc: false,
            receiving: false,
            batch_serial_no: it.batch_serial_no || "BATCH-001",
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const formItems = selectedRows.map((r) => ({
        ...
        rejected_quantity: Number(r.rejected_quantity || (Number(r.quantity || 0) - Number(r.accepted_quantity || 0))),
        batch_serial_no: r.batch_serial_no || "BATCH-001",
        remarks: r.remarks || "OK",
      }));
      ...
      // Determine QC status based on received vs accepted quantities
      let qcStatus = "PARTIAL";
      const allFullyAccepted = selectedRows.every(row => {
        const received = Number(row.quantity || 0);
        const accepted = Number(row.quantity || 0);
        return received === accepted && accepted > 0;
      });
      if (allFullyAccepted) {
        qcStatus = "FULL";
      }
      ...
      <div className="w-full max-w-4xl bg-white rounded-xl overflow-hidden shadow-lg">
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-3 overflow-x-auto max-h-80 border border-gray-200">
              <table className="min-w-full text-xs">
                <thead>
                  <tr className="text-left bg-gray-200 text-gray-700 sticky top-0">
                    <th className="px-3 py-2 min-w-32">Item Code</th>
                    <th className="px-3 py-2 min-w-40">Description</th>
                    <th className="px-3 py-2 w-24 text-center">Ordered Qty</th>
                    <th className="px-3 py-2 w-24 text-center">Received Qty</th>
                    <th className="px-3 py-2 w-24 text-center">Accepted Qty</th>
                    <th className="px-3 py-2 w-24 text-center">Rejected Qty</th>
                    <th className="px-3 py-2 min-w-28">Batch/Serial No.</th>
                    <th className="px-3 py-2 min-w-32">Remarks</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r, idx) => {
                    const orderedQty = Number(r.quantity || 0);
                    const receivedQty = Number(r.received_quantity || 0);
                    const acceptedQty = Number(r.accepted_quantity || 0);
                    ...
                    return (
                      <tr key={idx} className={`border-t text-xs ${selectedItems.includes(idx) ? 'bg-blue-50' : ''}`}>
                        ...
                        <td className="px-3 py-2 text-center">{orderedQty}</td>
                        <td className="px-3 py-2">
                          <input 
                            ...
                            onChange={(e) => {
                              const newRows = [...rows];
                              newRows[idx].received_quantity = Number(e.target.value) || 0;
                              setRows(newRows);
                            }}
                        ...
                        <td className="px-3 py-2">
                          <input 
                            ...
                            onChange={(e) => {
                              const newRows = [...rows];
                              newRows[idx].accepted_quantity = Number(e.target.value) || 0;
                              setRows(newRows);
                            }}
                            className="w-full border border-gray-300 rounded px-2 py-1 text-xs text-center"
                            min="0"
                        ...
                        <td className="px-3 py-2 text-center font-semibold bg-gray-100">{rejectedQty}</td>
                        <td className="px-3 py-2">
                          <input 
                            type="text" 
                            value={r.batch_serial_no || ""} 
                            onChange={(e) => {
                              const newRows = [...rows];
                              newRows[idx].batch_serial_no = e.target.value;
                              setRows(newRows);
                            }}

...
const fetchTableData = async (
  ...
  const handleRepair = async (unique_id) => {
    // 1. Get the userId and token from sessionStorage
    const userId = sessionStorage.getItem("userId");
    const token = sessionStorage.getItem("token"); // Add token
    ...
    try {
      ...
      const historyPayload = {
        ...
        previousStatus,
        currentStatus: "SendForRepair",
        previousStages,
        currentStages: "SendForRepair",
        action: "Asset Repair Request",
      };
      ...
      setMessage("Asset Repair Request Sent Successfully!");
  ...
  const handleRowClick = (asset) => {
    // Toggle between showing and hiding the full details
    if (selectedAsset && selectedAsset.unique_id === asset.unique_id) {
      setSelectedAsset(null); // If clicked again, close the details
    } else {
      setSelectedAsset(asset); // Show the full details for the clicked asset
    }
  };

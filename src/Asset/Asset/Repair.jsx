...
const fetchTableData = async (
  ...
  try {
    ...
    const params = {
      ...
      stages: "Damaged, Repaired, SendForRepair" ,
      sub_stages: "AwaitingApproval,Approved",
      type:"Movable",
    };
 ...
 const handleDiscard = async (unique_id) => {
  const userId = sessionStorage.getItem("userId");
  const token = sessionStorage.getItem("token"); // <-- Token get kiya
  ...
  const newSubStage = "AwaitingApproval";
  ...
  try {
    ...
    console.log("Discard request successful:", data);
    ...
    const historyRes = await fetch(
      `${JAVA_BASE}api/assethistory/insert-history`,
      {
        ...
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, // <-- Token yahan bhi
        },
...
const handleRepaired = async (unique_id) => {
  ...
  try {
    ...
    const data = await response.json();
    console.log("Repaired request submitted successfully:", data);

    // 2️⃣ Asset History Logging
    const historyPayload = {
      assetId: unique_id,
      categoryId: selectedCategoryId,
      updatedBy: parsedUserId,
    ...
    if (!historyRes.ok) {
     ...
     await fetchTableData(selectedCategory, pagination.offset, pagination.limit, searchTerm);
    setIsAssetModalOpen(false);
    setMessage("Asset Request successfully sent for Repaired!");
    setMessageType("success");
  } catch (error) {
    console.error("Repaired request failed:", error);
...
<div className="col-span-2 flex justify-end mt-4">
  {formData.stages === "Repaired" && formData.sub_stages === "Approved" ? (
    <button
      type="button"
      onClick={() => handleactive(editingAssetId)}
      className="bg-red-600 hover:bg-red-700 text-white py-2 px-6 rounded"
    >
      Asset Active Request
    </button>
  ) : formData.stages === "SendForRepair" &&

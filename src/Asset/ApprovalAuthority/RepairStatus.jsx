...
const handleDiscard = async (unique_id) => {
  ...
  if (!userId || !token) {
    ...
    setMessage("User not logged in or authentication token missing.");
  ...
  // 4. Make the API call to discard the asset
  try {
...
const handleRepaired = async (unique_id) => {
  ...
  try {
    ...
    const data = await response.json();
    console.log("Repaired approval successful:", data);
    ...
    const historyPayload = {
      ...
      action: "Asset Repaired Request Approved",
    };
    ...
    if (!historyRes.ok) {
     ...
     await fetchTableData(selectedCategory, pagination.offset, pagination.limit, searchTerm);
    setIsAssetModalOpen(false);
    setMessage("Asset Repaired Approved successfully!");
    setMessageType("success");
  } catch (error) {
    console.error("Repaired approval failed:", error);

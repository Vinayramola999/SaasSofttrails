...
//     // 3. Get asset details from tableData
...
//     // 4. Prepare the payload to update the asset status
//     const payload = {
...
//         action:"Asset Damage Request Approved",
//       };
  
//       // 6. Call the Asset History API
...
//       setMessage("Asset Damage Approved Successfully!");
...
//           {["Asset Name", "Created On", "Status", "Stage", "Sub Stage", "User Name", "Location"].map(
//             (column, index) => (
...
//             <tr
//               className={`cursor-pointer ${index % 2 === 0 ? "bg-blue-50" : "bg-white"} hover:bg-blue-100`}
//             >
//               {selectedCategory && (
...
//                 <td colSpan={selectedCategory ? 9 : 8} className="p-4">
...
//                       </div>
//                       <div>
//                         <strong>Mapped Location:</strong> {asset.location_details}
//                       </div>
//                       <div>
...
//                         type="button"
//                         onClick={() => {
//                           handleDamage(editingAssetId); // Call the approval handler
//                           setIsAssetModalOpen(false); // Close the modal after approval
//                         }}
...
/**
 * AssetManagementPage (final)
 *
 * - Single-file replacement for your old big component (NO dynamic-fields).
 * - Uses API: asset-mapping-detailss/{categoryId}
 * - Shows ONLY rows where:
 *      sub_stages === "AwaitingApproval"
 *      toapprove === "Mapping"
 *      stages === "Active"
 * - Table columns (Option-A): Asset Name | Type Of Allocation | Created On | Status | Stage | Sub Stage
 * - Approve button calls: lifecycle/update-asset-status/{id} (keeps your payload)
 * - Keeps the original table look + pagination UI you asked for.
 * - Modal "Asset Review" matches the design you provided, with colored badges.
 */
...
const AssetManagementPage = () => {
  ...
  const [categories, setCategories] = useState([]);
  const [selectedCategory, setSelectedCategory] = useState("");
  const [tableData, setTableData] = useState([]); // rows to display
  const [page, setPage] = useState(1);
  const [limit] = useState(15); // page size; matches your older UI
  ...
  // keep stable badge colors during session
  const badgeColorRef = useRef({});
  ...
  const getBadgeClass = (key) => {
    if (!key) return "text-gray-700";
    if (!badgeColorRef.current[key]) badgeColorRef.current[key] = pickBadgeClass(key);
    return badgeColorRef.current[key];
  };
...
const fetchTableData = async () => {
  try {
    ...
    const tp =
      ...
      Math.ceil(
        (res.data?.pagination?.totalRecords || raw.length) / limit
      ) ||
      1;
  ...
  const handleApprove = async () => {
    try {
      ...
      setMessageType("success");
      setIsAssetModalOpen(false);
      // refresh table (will reapply filter)
      fetchTableData();
    } catch (err) {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {tableData.length === 0 ? (
                ...
                tableData.map((row, idx) => (
                  <tr key={row.unique_id} className={`${idx % 2 === 0 ? "bg-blue-50" : "bg-white"} hover:bg-blue-100 cursor-pointer`}>
                    ...
                    {tableColumns.map((col) => (
                      <td key={col.key} className="px-5 py-3 text-center">
                        {col.key === "created_at" ? (
                          ...
                          <span className={`inline-block px-2 py-1 rounded-full text-sm font-medium ${getBadgeClass(row[col.key])}`}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <p>
                    <strong>Asset Name:</strong> {safe(selectedAsset["Asset Name"] || selectedAsset.name)}
                  </p>
                  <p>
                    <strong>Status:</strong> {safe(selectedAsset.status)}
                  </p>
                  <p>
                    <strong>Stage:</strong>{" "}
                    <span className={`inline-block px-2 py-1 rounded-full text-sm font-medium ${getBadgeClass(selectedAsset.stages)}`}>
                      {formatStages(selectedAsset.stages)}
                    </span>
                  </p>
                  <p>
                    <strong>Sub Stage:</strong>{" "}
                    <span className={`inline-block px-2 py-1 rounded-full text-sm font-medium ${getBadgeClass(selectedAsset.sub_stages)}`}>
                      {formatStages(selectedAsset.sub_stages)}
                    </span>
                  </p>
                  <p>
                    <strong>Created:</strong> {fDate(selectedAsset.created_at)}
                  </p>
                  <p>
                    <strong>Mapping Status:</strong>{" "}
                    <span className={`inline-block px-2 py-1 rounded-full text-sm font-medium ${getBadgeClass(selectedAsset.mapping_status)}`}>
                      {safe(selectedAsset.mapping_status)}
                    </span>
                  </p>
                </div>
              </div>
              ...
              {selectedAsset.allocation_type === "allocation to user" && (
                <div>
                  ...
                  <p>
                    <strong>User:</strong> {safe(selectedAsset.allocated_user_name)}
                  </p>
                  <p>
                    <strong>Mapping ID:</strong> {safe(selectedAsset.mapping_id)}
                  </p>
                </div>
              )}
              ...
              {selectedAsset.allocation_type === "allocation to location" && (
                <div>
                  ...
                  <p>
                    <strong>Address:</strong> {safe(selectedAsset.location_full_address || selectedAsset.location_master_address)}
                  </p>
                  <p>
                    <strong>Building/Floor/Room:</strong> {`${safe(selectedAsset.building_no)} / ${safe(selectedAsset.floor)} / ${safe(selectedAsset.room)}`}
                  </p>
                  <p>
                    <strong>Mapping ID:</strong> {safe(selectedAsset.mapping_id)}
                  </p>
              ...
              {selectedAsset.allocation_type === "asset to asset allocation" && (
                <div>
                  ...
                  <p>
                    <strong>Source:</strong> {safe(selectedAsset.source_asset_name)}
                  </p>
                  <p>
                    <strong>Destination:</strong> {safe(selectedAsset.destination_asset_name)}
                  </p>
                  <p>
                    <strong>Mapping ID:</strong> {safe(selectedAsset.mapping_id)}
                  </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <p>
                    <strong>Purchase Date:</strong> {fDate(selectedAsset["Purchase Date"])}
                  </p>
                  <p>
                    <strong>Original Cost:</strong> {safe(selectedAsset["Original Cost"])}
                  </p>
                  <p>
                    <strong>Scrap Value:</strong> {safe(selectedAsset["Scrap Value"])}
                  </p>
                  <p>
                    <strong>Useful Life:</strong> {safe(selectedAsset["Useful Life"])}
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3">
                <button onClick={handleApprove} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">
                  Approve
                </button>
                <button onClick={() => setIsAssetModalOpen(false)} className="px-4 py-2 rounded border">
                  Close
                </button>
              </div>
            </div>
